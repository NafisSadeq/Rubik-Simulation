...
public class PhotoCube {
    ...
    private float cubeHalfSize = .6f;

    public void change_color(Context context){
        ...
        for (int face = 0; face < numFaces; face++) {
            bitmap[face] = BitmapFactory.decodeStream(
                    context.getResources().openRawResource(imageFileIDs[color[face]]));
            int imgWidth = bitmap[face].getWidth();
            int imgHeight = bitmap[face].getHeight();
            float faceWidth = 1.2f;
            float faceHeight = 1.2f;
    ...
    public PhotoCube(Context context,int front,int back,int left,int right,int up,int down) {
